...
```{r}
k <- 1e5
sampled <- data.frame(proposal = runif(k, 0, 1))
sampled$targetDensity <- dbeta(sampled$proposal, alpha, beta)
maxDens = max(sampled$targetDensity, na.rm = TRUE)
sampled$accepted = ifelse(runif(k, 0, 1) < sampled$targetDensity / maxDens, TRUE, FALSE)

hist(sampled$proposal[sampled$accepted], freq = FALSE, col = "grey", breaks = 100)
curve(dbeta(x, alpha, beta), 0, 1, add = TRUE, col = "red")
```
...
par(new = TRUE)
hist(sampled$proposal[sampled$accepted], freq = FALSE, col = "red", breaks = 100, xlab = "", ylab = "",
     xaxt = "n", yaxt = "n", main = "")
````
...
sampled$targetDensity <- dbeta(sampled$proposal, alpha, beta)
maxDens = max(sampled$targetDensity, na.rm = TRUE)
sampled$accepted = ifelse(runif(k, 0, 1) < sampled$targetDensity, TRUE, FALSE)
...
```{r}
hist(sampled$proposal, freq = FALSE, col = "grey", breaks = 100) 
par(new = TRUE)
hist(sampled$proposal[sampled$accepted], freq = FALSE, col = "red", 
...
## TODO
This is too simplistic and needs to be written down more formal with the respective math